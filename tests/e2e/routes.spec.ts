...
async function loadDataset(page: Page) {
	...
	await Promise.all([
		page.waitForResponse(
			(response: Response) =>
				response.url().includes("/data/elgoose_setlists.json") && response.ok(),
			{
...
async function openCoverArtists(page: Page) {
	await page
		.getByRole("link", { name: /Browse & search cover artists/i })
		.click();
	await expect(page).toHaveURL(/\/#\/covers$/);
	await expect(
		page.getByRole("heading", { name: "Cover Artists" }),
	).toBeVisible();
...
async function waitForTopShowsStable(page: Page) {
...
test("show detail route renders setlist for selected show", async ({
	page,
}) => {
	await loadDataset(page);
	const firstRow = page.locator("table tbody tr").first();
...
test("song detail route shows statistics for selected song", async ({
	page,
}) => {
	await loadDataset(page);
	const firstRow = page.locator("table tbody tr").first();
...
test("clear cache resets dashboard state", async ({ page }) => {
	...
	await expect(
		page.locator("text=No shows match the selected filters."),
	).toBeVisible();
...
test("cover artists table supports sorting toggles", async ({ page }) => {
	...
	for (let i = 0; i < totalsInitial.length - 1; i += 1) {
		const current = totalsInitial[i]!;
		const next = totalsInitial[i + 1]!;
	...
	for (let i = 0; i < totalsAscending.length - 1; i += 1) {
		const current = totalsAscending[i]!;
		const next = totalsAscending[i + 1]!;
	...
	const sortedNames = [...namesAscending].sort((a, b) =>
...
test("cover artist detail metrics and song sorting", async ({ page }) => {
	...
	const search = page.getByPlaceholder(
		"Search artists (e.g. Perpetual Groove)",
	);
	await search.fill("Grateful Dead");
	await expect(
		rows.filter({ hasText: /Grateful Dead/i }).first(),
	).toBeVisible();

	await rows
		.filter({ hasText: /Grateful Dead/i })
		.first()
		.click();
	...
	const initialSongFirst = await songRows
		.first()
		.locator("td")
		.first()
		.innerText();
	...
	const songNameCollator = new Intl.Collator(undefined, {
		sensitivity: "base",
		ignorePunctuation: true,
	});
	await expect
		.poll(async () => {
			...
			for (let i = 0; i < names.length - 1; i += 1) {
				...
				if (songNameCollator.compare(current, next) > 0) {
					return false;
				}
			}
			return true;
	...
	if (songRowCount > 1) {
		...
		for (let i = 0; i < songRowCount - 1; i += 1) {
			const current = (
				await songRows.nth(i).locator("td").first().innerText()
			).trim();
			const next = (
				await songRows
					.nth(i + 1)
		...
		expect(
			(await songRows.first().locator("td").first().innerText()).trim(),
		).toEqual(initialSongFirst.trim());
	}
	...
	if (songRowCount > 1) {
		for (let i = 0; i < songRowCount - 1; i += 1) {
			const current = Number.parseInt(
				await songRows.nth(i).locator("td").nth(1).innerText(),
				10,
			);
...
test("song index supports filtering and sorting", async ({ page }) => {
	...
	for (let i = 0; i < initialRarity.length - 1; i += 1) {
		const current = initialRarity[i]!;
		const next = initialRarity[i + 1]!;
		expect(current).toBeGreaterThanOrEqual(next);
	}
	...
	for (let i = 0; i < Math.min(nameCount, 10); i += 1) {
		const name = (
			await rows.nth(i).locator("td").first().locator("div").first().innerText()
		).trim();
		names.push(name);
	}
	...
	await expect
		.poll(async () => {
			...
			return (
				texts.length > 0 && texts.every((text) => text.includes("Cover •"))
			);
		})
		.toBeTruthy();
	...
	const firstSongName = (
		await rows.first().locator("td").first().locator("div").first().innerText()
	).trim();
	await rows.first().click();
	await expect(page).toHaveURL(/\/#\/songs\//);
	await expect(
		page.getByRole("heading", { name: firstSongName }),
	).toBeVisible();
	await expect(
		page.getByRole("heading", { name: "Appearances" }),
	).toBeVisible();
});
...
test("song duration sorting orders appearances by length", async ({ page }) => {
	...
	if (await loadButton.isVisible()) {
		await Promise.all([
			page.waitForResponse(
				(response) =>
					response.url().includes("/data/elgoose_setlists.json") &&
					response.ok(),
				{ timeout: FAST_TIMEOUT },
			),
			loadButton.click(),
	...
	await expect(occurrencesTbody.locator("tr").first()).toBeVisible({
		timeout: FAST_TIMEOUT,
	});
	...
	function parseDuration(text: string): number {
		...
		return parts.reduce(
			(total, part) => total * 60 + (Number.isNaN(part) ? 0 : part),
			0,
		);
	}

	function assertDescending(durations: readonly number[]) {
	...
	function assertAscending(durations: readonly number[]) {
		for (let i = 0; i < durations.length - 1; i += 1) {
			const current = durations[i]!;
			const next = durations[i + 1]!;
			expect(current).toBeLessThanOrEqual(next);
...
test("song detail view highlights setlist groups and rarity", async ({
	page,
}) => {
	await loadDataset(page);
	...
	const missionOccurrences = await occurrenceRows
		.filter({ hasText: /Mission Ballroom/i })
		.count();
