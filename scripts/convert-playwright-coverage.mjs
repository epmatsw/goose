...
const DEFAULT_INPUT = path.resolve(ROOT_DIR, 'playwright-coverage/coverage.json');
const DEFAULT_OUTPUT_DIR = path.resolve(ROOT_DIR, 'playwright-coverage/istanbul');
...
async function main() {
  try {
    ...
    if (!(await fileExists(options.input))) {
      console.error(`[convert-playwright-coverage] Input file not found: ${options.input}`);
      process.exitCode = 1;
      return;
    }
    ...
    if (entries.length === 0) {
      console.warn('[convert-playwright-coverage] No coverage entries found in input file.');
      return;
    }

    const { coverageMap, skipped } = await convertCoverage(entries);
    if (coverageMap.files().length === 0) {
      console.warn('[convert-playwright-coverage] No matching source files were covered.');
      if (skipped.size > 0) {
        console.warn(
    ...
    console.log('[convert-playwright-coverage] Reports written to', options.outDir);
    if (skipped.size > 0) {
      console.log(
