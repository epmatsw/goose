...
function isCover(entry) {
  const originalFlag = normalizeIsOriginal(entry?.isoriginal);
  return originalFlag === undefined ? Boolean(entry?.original_artist) : !originalFlag;
}
...
function computePlayCounts(setlists) {
...
function groupSetlistsByShow(setlists) {
...
function computeRarityScores(dataset) {
  ...
  const playCounts = computePlayCounts(setlists);
  ...
  const rawRarities = setlists.map((entry) => {
    const key = songKey(entry);
    const plays = playCounts.get(key) ?? 1;
    const base = Math.min(1 / plays, 1 / F_CAP);
  ...
  for (const show of shows) {
    ...
    const rarityScore = Math.max(total, MIN_SHOW_SCORE);
    scores.push({
      showId: show.show_id,
      date: show.showdate ?? null,
      venue: show.venuename ?? '',
