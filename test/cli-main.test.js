...
describe("goose-rarity CLI runner", () => {
	...
	it("runs the CLI end-to-end using an in-memory dataset", async () => {
		...
		expect(logSpy).toHaveBeenCalledWith(
			expect.stringContaining(
				"Using dataset provided via ELGOOSE_DATASET_JSON.",
			),
		);
		expect(logSpy).toHaveBeenCalledWith(
			expect.stringContaining(
				"show(s) omitted because no setlist data was available.",
			),
		);
		expect(logSpy).toHaveBeenCalledWith(
			expect.stringContaining("Rarity scores written to rarity.csv"),
		);
		expect(logSpy).toHaveBeenCalledWith(
			expect.stringContaining("Average rarity score across"),
		);
	...
	it("logs a warning and writes an empty CSV when filters remove all shows", async () => {
		...
		expect(exitCode).toBe(0);
		expect(logSpy).toHaveBeenCalledWith(
			expect.stringContaining("No shows found for year 1999"),
		);
	...
	it("returns a non-zero exit code when required dataset fields are missing", async () => {
		...
		expect(errorSpy).toHaveBeenCalledWith(
			expect.stringContaining("Failed to compute rarity scores"),
		);
	});
