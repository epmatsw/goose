...
async function fetchSetlistForShow(showId: number): Promise<JsonRecord[]> {
	const entries = await fetchJson<JsonRecord[]>(
		`setlists/show_id/${showId}.json`,
	);
...
function createSetlistEntryKey(
	entry: JsonRecord | undefined | null,
): string | undefined {
	...
	const songId =
		entry.song_id ?? entry.songid ?? entry.songhistoryid ?? "unknown";
	const position =
		entry.position ??
		entry.sortorder ??
		entry.setorder ??
	...
	if (newShowRecords.length > 0) {
		...
		async function worker() {
			while (index < newShowRecords.length) {
				...
				index += 1;
				const record = newShowRecords[currentIndex];
				if (!record) {
					continue;
				}
				const { id } = record;
		...
		const workerCount = Math.min(
			SETLIST_FETCH_CONCURRENCY,
			Math.max(1, newShowRecords.length),
		);
		...
		if (errors.length > 0) {
			const detail = errors
				...
				.join("; ");
			throw new Error(
				`Failed to download setlists for ${errors.length} new show(s): ${detail}`,
			);
		}
